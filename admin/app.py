...
def generate_invoice_pdf(invoice_number, order, customer):
    ...
    # Zkrácení variabilního symbolu stejným způsobem jako v JS funkci
    short_vs = var_symbol[:8] + var_symbol[-2:] if len(var_symbol) > 10 else var_symbol
    draw_czech_text(p, 200, height - 240, short_vs, style='medium')
    ...
    # QR kód pro platbu podle specifikace - upraveno pro shodu s JavaScript funkcí
    iban = 'CZ1830300000002411153019'  # IBAN ve správném formátu
    formattedAmount = f"{order['total']:.2f}"
    message = f"Objednávka číslo #{order['order_number']}"
    
    # Použití stejného zkráceného variabilního symbolu jako výše
...
def send_invoice_email_direct(invoice_info, file_path, customer_data, order_data, company_info):
    ...
    password = "!5"      # Přímé heslo (pro účely testování)
